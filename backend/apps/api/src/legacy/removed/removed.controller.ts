...
import { RemovedMessagesService } from './removed.service';
import { ExportQuerySchema, ListQuerySchema } from '../dto/list-query.dto';
import { toCsv, formatBrasilia } from '../csv.util';

@Controller('v2/legacy/removed')
export class RemovedMessagesController {
  constructor(private readonly service: RemovedMessagesService) {}
  ...
  async exportCsv(@Query() query: Record<string, string>, @Res() res: Response) {
    ...
    const csv = toCsv(rows, [
      ...
      { key: 'deletedMessage', header: 'deletedMessage' },
      {
        key: 'timestamp',
        header: 'timestamp',
    ...
    res.setHeader('Content-Disposition', `attachment; filename="removed-${Date.now()}.csv"`);
    res.send(csv);
  }
}
